...
void set_keyboard(int row, int bit)
{
    ...
    send_command(keybuf, sizeof(keybuf));
}

void unset_keyboard(int row, int bit)
{
    keybuf[row] &= (~(1 << bit)) & 0xff;

    send_command(keybuf, sizeof(keybuf));
}

std::map<SDL_Keycode, FmKeyLoc_t> sKeyMap;
...
DWORD WINAPI draw_run(void *arg) {
    ...
    auto set_title = [&]() {
        char tmp[100];
        snprintf(tmp, sizeof(tmp), "Family Basic Keyboard Emulator");
    ...
    // Create window
    window = SDL_CreateWindow("Family Basic Keyboard Emulator", SDL_WINDOWPOS_UNDEFINED, SDL_WINDOWPOS_UNDEFINED, 640, 480, SDL_WINDOW_SHOWN | SDL_WINDOW_RESIZABLE);
    if (window == NULL) {
    ...
    while (usb_run_flag) {
        ...
        SDL_WaitEvent(&e);
            if (e.type == SDL_QUIT) {
                ...
                auto it = sKeyMap.find(e.key.keysym.sym);
                if (it != sKeyMap.end()) {
...
int main(int argc, char *argv[]) {
    ...
    for (KeyMap_t m : gKeyMap) {
        sKeyMap.insert(std::make_pair(m.key, m.Loc));
    }
